...
void generate_structure_report(const Factory& f, std::ostream& os){
    ...
    for (auto worker = f.worker_cbegin(); worker != f.worker_cend(); ++worker) {
        ...
        os << "  Queue type: "
        << static_cast<std::underlying_type<PackageQueueType>::type>(worker->get_queue()->get_queue_type())
        << std::endl;
        os << "  Receivers:" << std::endl;
        for (const auto&[key, value] : worker->receiver_preferences_.get_preferences()) {
            os << "    " << static_cast<std::underlying_type<ReceiverType>::type>(key->get_receiver_type()) << " #"
            << key->get_id() << std::endl;
        }
    }
...
void generate_simulation_turn_report(Factory f, std::ostream& os, Time T){
    ...
    for (auto worker = f.worker_begin(); worker != f.worker_end(); ++worker){
        os << "WORKER #" << worker->get_id() << std::endl;
